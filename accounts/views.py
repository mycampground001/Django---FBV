...
def signup(request):
    ...
    if request.method == "POST":
        ...
        if user_form.is_valid():
            user = user_form.save()#dB 에 저장
            auth_login(request,user)
            messages.info(request,f'{user.username}님,가입 성공!')
            return redirect('boards:index')
        messages.warning(request, '양식을 다시 확인해라')
    else:
...
def login(request):
    ...
    if request.method == "POST":
        ...
        if login_form.is_valid():
            auth_login(request, login_form.get_user()) #DB에 저장하는 것이 아님
            return redirect('boards:index')
    else:
